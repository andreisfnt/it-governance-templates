# IT Governance Templates

A collection of reusable IT governance documents for IT leaders, operations teams, and IT managers. These templates are generic by design - adaptable to organizations of any size, sector, or maturity level.

Replace placeholder text in `[brackets]` with your organization's specifics. All templates are vendor-neutral and technology-agnostic.

---

## Templates

| Template | Category | Purpose |
|----------|----------|---------|
| [Employee IT Onboarding Checklist](onboarding/employee-it-onboarding-checklist.md) | Onboarding | Structured checklist for provisioning and onboarding new employees |
| [Runbook Template](operations/runbook-template.md) | Operations | Standard format for operational runbooks and IT procedures |
| [Vendor Evaluation Framework](vendor-management/vendor-evaluation-framework.md) | Vendor Management | Scoring framework for evaluating and selecting technology vendors |
| [IT Budget Planning Template](finance/it-budget-planning-template.md) | Finance | Budget structure and planning guide for IT departments |
| [SLA Template](service-management/sla-template.md) | Service Management | Service Level Agreement template for internal or external IT services |

---

## Principles behind these templates

**Completeness over brevity.** A template that skips the hard parts isn't useful. These are designed to surface the questions that matter, not just the easy ones.

**Process, not tooling.** These documents describe what good looks like. How you implement it depends on your environment.

**Usable from day one.** Each template is self-contained. No dependencies, no prerequisites beyond a text editor.

---

Maintained by [Andrei Pasca](https://github.com/andreisfnt)
